...
class ComponentCommand extends AbstractCommand {
  ...
  run() {
    ...
    this._deps = this.getOption('depends-on');
  ...
  _addExistingComponent() {
    ...
    componentData.component['depends_on'] = this._deps;
  ...
  _createNewComponent() {
    ...
    return Promise.all(
      ...
      return renderTwig(this._srcFile, { name: this._name, deps: this._deps }, outFile);
    }).then(() => 'Done');
  }
